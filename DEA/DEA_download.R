...
# download gtex RNA-Seq
for(i in 1:nrow(gtex_path)) {
    gtex_meta<-str_match(gtex_path[i, 1],'gtex.gtex.(.*?).MD')[2]
    try(
    rse_gene <- recount3::create_rse_manual(
    project = gtex_meta,
    ...
    type = "gene"))
    try(assays(rse_gene)$counts <- transform_counts(rse_gene))
    try(fn <- paste0("/localdisk/home/s2172876/project/Count/",gtex_meta,".gz"))
    try(write.table(assays(rse_gene)$counts,gzfile(fn)))
