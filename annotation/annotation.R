...
# filter group contains only on sample
...
for(i in unique(Group$study)){
    summary_level<-vector()
    for(j in 1:8){
    summary_level <-c(summary_level,length(table(Group[which(Group$study == i),j])))
    }
...
factors_summary
ggplot(data = factors_summary, mapping = aes(x = Type, y = Number)) + 
        geom_bar(stat = 'identity',fill="#277ac8", color="#e9ecef", width=0.7)+
        theme(axis.text=element_text(size=12,face="bold"),plot.title = element_text(hjust = 0.5))+
        labs(title = "Project type")+
        scale_x_discrete(labels=function(x) str_wrap(x, width=14))+
        geom_text(aes(label = Number), vjust = 1.5, colour = "white",fontface='bold')
ggsave(file="experiment_type.pdf")

# t <- vector()
# for(i in c(two_factor,three_factor,more_than_three_factor)){
#     summary_level<-vector()
#     for(j in 4:8){
#     summary_level <-c(summary_level,length(table(Group[which(Group$study == i),j])))
#     }
#     t <- c(t,length(which(summary_level !=1)))
# }
# length(which(t>2))
# table(t)
